...
class UserCreatorService
{
    ...
    public function make(array $attributes)
    {
        if( $this->validator->isValidForInsert($attributes) )
        {
            ...
            if ( $user->exists() )
            {
                $user->profiles()->create($attributes['profile']);
                return $user->first();
            }
            else
            {
    ...
    protected function makeWithProfile($attributes)
    {
        if( $this->validator->isValidForInsert($attributes) )
        {
            $user = \DB::transaction(function () use ($attributes) {
                $attributes['password'] = bcrypt($attributes['password']) ?: bcrypt(str_random(8));

                return User::create($attributes)->profiles()->create($attributes['profile']);
            });
